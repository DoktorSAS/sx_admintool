...
akhp = Config.AutoKickHighPing
kickWarning = true
Citizen.CreateThread(function()
    if Config.AutoKickAFK then
	while true do
		Wait(1000)
        ...
        if playerPed then
            ...
            if akhp then
                TriggerServerEvent("SX:CheckPing")
            end
			if currentPos == prevPos then
				if time > 0 then
					if kickWarning and time == math.ceil(secondsUntilKick / 4) then
						TriggerEvent("chatMessage", "^7[^5SXAdmin^7]", {255, 0, 0}, "^1You'll be kicked in " .. time .. " seconds for being AFK!")
					end
					time = time - 1
				else
					TriggerServerEvent("SX:AFK")
				end
			else
...
-- Consume fuel factor
fuelmanager = Config.FuelManager
Citizen.CreateThread(function()
	if fuelmanager then
	while true do
		local Ped = GetPlayerPed(-1)
